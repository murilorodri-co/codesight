...
export default function Produtos() {
  ...
  const produtosIniciais = [
    { nome: "Produto 1", ncm: "8471.30.12", preco: "R$ 50,00", unidade: "Litros", materias: "Ver matérias-primas", materiasPrimas: ["Água", "Corante Azul", "Conservante"], status: "Fora de estoque", variacao: "Ver variações", adicionado: "04/09/2025" },
    { nome: "Produto 2", ncm: "1234.56.78", preco: "R$ 100,00", unidade: "Caixa", materias: "Ver matérias-primas", materiasPrimas: ["Papelão", "Tinta", "Cola"], status: "Em estoque", variacao: "Ver variações", adicionado: "05/09/2025" },
    { nome: "Produto 3", ncm: "5678.90.12", preco: "R$ 75,00", unidade: "Unidade", materias: "Ver matérias-primas", materiasPrimas: ["Plástico", "Metal", "Parafuso"], status: "Pendente", variacao: "Ver variações", adicionado: "06/09/2025" },
    { nome: "Produto 4", ncm: "9101.11.13", preco: "R$ 250,00", unidade: "Kg", materias: "Ver matérias-primas", materiasPrimas: ["Cimento", "Areia", "Brita"], status: "Fora de estoque", variacao: "Ver variações", adicionado: "07/09/2025" },
    { nome: "Produto 5", ncm: "1415.16.17", preco: "R$ 15,00", unidade: "Pacote", materias: "Ver matérias-primas", materiasPrimas: ["Farinha", "Açúcar", "Fermento"], status: "Em estoque", variacao: "Ver variações", adicionado: "08/09/2025" },
    { nome: "Produto 6", ncm: "1819.20.21", preco: "R$ 320,00", unidade: "Litros", materias: "Ver matérias-primas", materiasPrimas: ["Óleo Mineral", "Corante Vermelho", "Essência"], status: "Fora de estoque", variacao: "Ver variações", adicionado: "09/09/2025" },
    { nome: "Produto 7", ncm: "2223.24.25", preco: "R$ 8,50", unidade: "Unidade", materias: "Ver matérias-primas", materiasPrimas: ["Vidro", "Tampa de Metal", "Etiqueta"], status: "Em estoque", variacao: "Ver variações", adicionado: "10/09/2025" },
    { nome: "Produto 8", ncm: "2627.28.29", preco: "R$ 180,00", unidade: "Kg", materias: "Ver matérias-primas", materiasPrimas: ["Algodão", "Tinta", "Botões"], status: "Em estoque", variacao: "Ver variações", adicionado: "11/09/2025" },
    { nome: "Produto 9", ncm: "3031.32.33", preco: "R$ 90,00", unidade: "Caixa", materias: "Ver matérias-primas", materiasPrimas: ["Chapas de Aço", "Parafusos", "Solda"], status: "Pendente", variacao: "Ver variações", adicionado: "12/09/2025" }
];
  ...
  const colunas = [
    ...
    { chave: "materias", titulo: "Ver matérias-primas" },
  ...
  const exportPDF = () => {
    ...
    doc.text("Relatório de Produtos", pageWidth / 2, y, { align: "center" });
  
    y += 10;
  
    // Data atual no canto direito
    const date = new Date().toLocaleDateString();
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text(`Data: ${date}`, pageWidth - margin, 20, { align: "right" });
  
    y += 10;
  
    produtosIniciais.forEach((p, index) => {
      ...
      doc.setFontSize(12);
      doc.setFont("helvetica", "bold");
      doc.text(`${index + 1}. ${p.nome}`, margin, y);
      y += 6;
      ...
      const detalhes1 = `NCM: ${p.ncm}   |   Preço: ${p.preco}   |   Unidade: ${p.unidade}`;
      const detalhes1Lines = doc.splitTextToSize(detalhes1, pageWidth - 2 * margin);
      detalhes1Lines.forEach(line => {
        doc.text(line, margin, y);
        y += 5;
      });
  
      const materiasText = Array.isArray(p.materiasPrimas) ? p.materiasPrimas.join(", ") : "";
      const detalhes2 = `Matérias-primas: ${materiasText}   |   Status: ${p.status}   |   Adicionado: ${p.adicionado}`;
      const detalhes2Lines = doc.splitTextToSize(detalhes2, pageWidth - 2 * margin);
      detalhes2Lines.forEach(line => {
        doc.text(line, margin, y);
        y += 5;
      });
  ...
  // Estados
  const [isModalEditarOpen, setIsModalEditarOpen] = useState(false); // já existente
  const [isModalMateriasOpen, setIsModalMateriasOpen] = useState(false); // novo modal


  const openMateriasModal = (produto) => {
    setSelectedProduct(produto);
    setIsModalMateriasOpen(true);
  };
  
  const closeMateriasModal = () => {
    setSelectedProduct(null);
    setIsModalMateriasOpen(false);
  };
      ...
      <section className="section-stock-main">
        ...
        <div className="section-stock-table">
          <table className="stock-table-main">
              ...
              {produtosPagina.map((produto, index) => (
                <tr key={index}>
                  ...
                  <td
                    ...
                    onClick={() => {
                      setSelectedProduct(produto);       // salva o produto clicado
                      setIsModalMateriasOpen(true);      // abre o modal certo
                    }}
                  >
                    {produto.materias}
                  </td>
            ...
            {/* Modal de matérias-primas */}
            {isModalMateriasOpen && selectedProduct && (
              <div className="stock-modal-overlay" onClick={() => setIsModalMateriasOpen(false)}>
                <div
                  className="stock-modal-content"
                  onClick={(e) => e.stopPropagation()}
                >
                  <h2>Matérias-primas de {selectedProduct.nome}</h2>
                  {selectedProduct.materiasPrimas && selectedProduct.materiasPrimas.length > 0 ? (
                    <ul style={{listStyle: 'none'}}>
                      {selectedProduct.materiasPrimas.map((materia, i) => (
                        <li key={i} style={{ margin: "0.5rem 0", fontWeight: "500" }}>
                          {materia}
                        </li>
                      ))}
                    </ul>
                  ...
                  <div className="stock-modal-actions">
                    <button
                      className="stock-modal-cancel"
                      onClick={() => setIsModalMateriasOpen(false)}
                    >
                      Fechar
                    </button>
                  </div>
  ...
  // Ignora o campo "variacao" e o "materias" (aquele "Ver matérias-primas")
  if (campo === "variacao" || campo === "materias") return null;

  const coluna = colunas.find(c => c.chave === campo);

  // Campo de matérias-primas em input formatado
  if (campo === "materiasPrimas") {
    return (
      <div key={campo} className="stock-modal-field">
        ...
        <input
          ...
          name={campo}
          value={
            Array.isArray(selectedProduct[campo])
              ? selectedProduct[campo].join(", ")
