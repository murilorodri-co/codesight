...
export default function MateriasPrimas() {
  ...
  const [isAddNotaFiscalModalOpen, setIsAddNotaFiscalModalOpen] = useState(false);
  ...
  const materiaisFiltrados = materiais
    ...
    .filter((materia) =>
      filtros.unidade ? materia.unidade === filtros.unidade : true
    )
    .filter((materia) =>
      filtros.variacao ? materia.variacao.toLowerCase().includes(filtros.variacao.toLowerCase()) : true
    )
    .filter((materia) =>
      filtros.adicionado ? materia.adicionado.toLowerCase().includes(filtros.adicionado.toLowerCase()) : true
    )
    .filter((materia) =>
      filtros.status ? materia.status === filtros.status : true
    );
  ...
  const exportPDF = () => {
    ...
    materiaisFiltrados.forEach((m, index) => {
      doc.text(
        `${index + 1}. ${m.nome} - ${m.ncm} - ${m.preco} - ${m.unidade} - ${m.variacao} - ${m.possuiNF} - ${m.adicionado}`,
        14,
        y
      );
      y += 10;
  ...
  const handleChange = (chave, valor) => {
    setManualData({ ...manualData, [chave]: valor });
  };

  const handleSubmit = (e) => {
    e.preventDefault();
    console.log("Dados cadastrados:", manualData);
    setIsAddManualModalOpen(false);
    // Aqui você pode adicionar lógica para salvar os dados na tabela ou backend
  };
        ...
        <div className="section-stock-actions">
          ...
          <div className="section-actions-buttons">
                  ...
                  <form>
                    ...
                    <div className="filter-field">
                      ...
                      <select
                        ...
                        onChange={(e) => aplicarFiltro("unidade", e.target.value)}
                      >
                        <option value="">Todos</option>
            ...
            <button
              ...
              onClick={() => setIsAddNotaFiscalModalOpen(true)}
            >
            ...
            {isAddNotaFiscalModalOpen && (
              <div
                className="export-modal-overlay"
                onClick={() => setIsAddNotaFiscalModalOpen(false)}
              >
                <div className="stock-modal-content">
                  ...
                  <h2>Escolha como adicionar a matéria prima</h2>
                  <div className="stock-modal-buttons">
                    <button onClick={() => setIsAddModalOpen(true)}>
                      Adicionar com nota fiscal
                    </button>
                    <button onClick={() => setIsAddManualModalOpen(true)}>
              ...
              >
                ...
                >
                  ...
                  <form onSubmit={handleSubmit}>
                    {colunas.map((col) => {
                      // Campo "possuiNF" como checkbox
                      if (col.chave === "possuiNF") {
                        return (
                        <div key={col.chave} className="stock-modal-field checkbox-field">
                          <span>{col.titulo}</span>
                          <label className="custom-checkbox">
                            <input
                              type="checkbox"
                              checked={manualData.possuiNF}
                              onChange={(e) => handleInputChange(col.chave, e.target.checked)}
                            />
                      ...
                      if (col.chave === "adicionado") {
                        return (
                          <div key={col.chave} className="stock-modal-field">
                            <label>{col.titulo}</label>
                            <input type="text" value={manualData.adicionado} readOnly />
                          </div>
                        );
                      }

                      // Campo "preco" como número formatado em R$
                      if (col.chave === "preco") {
                        return (
                          <div key={col.chave} className="stock-modal-field">
                            <label>{col.titulo}</label>
                      ...
                      // Campo "unidade" como select
                      if (col.chave === "unidade") {
                        return (
                          <div key={col.chave} className="stock-modal-field">
                            <label>{col.titulo}</label>
                            <select
                              value={manualData.unidade}
                              onChange={(e) =>
                                handleChange(col.chave, e.target.value)
                              }
                            >
                              <option value="Kg">Kg</option>
                              <option value="g">g</option>
                    ...
                    <div className="stock-modal-actions" style={{ marginTop: "20px" }}>
                      <button
                        type="button"
                        className="stock-modal-cancel"
                        onClick={() => setIsAddManualModalOpen(false)}
                      ...
                      <button type="submit" className="stock-modal-save">
                        Salvar
                      </button>
                    </div>
                  </form>
                ...
                {isEditVariacaoModalOpen && (
                  <div className="stock-modal-overlay" onClick={fecharModalEditarVariacao}>
                    <div className="stock-modal-content" onClick={(e) => e.stopPropagation()}>
                      ...
                      <form>
                        {["nome", "ncm", "preco", "unidade", "variacao", "possuiNF", "adicionado"].map((campo) => (
